import { UpdatePetContactUseCase } from '../update-pet-contact.use-case'
...
describe('UpdatePetContactUseCase', () => {
  ...
  it('atualiza um contato do pet', async () => {
    ...
    const useCase = new UpdatePetContactUseCase(pets, featureAccess, contacts)

    const result = await useCase.execute('user-1', 'pet-1', 'c-1', {
      name: 'Maria S.',
    })

    expect(result.name).toBe('Maria S.')
    expect(contacts.save).toHaveBeenCalledWith(c)
  })

  it('lança PetContactNotFoundError se o contato não existe', async () => {
    pets.findById.mockResolvedValue(makePet('user-1'))
    featureAccess.hasFeature.mockResolvedValue(true)
    ...
    await expect(
      useCase.execute('user-1', 'pet-1', 'c-x', { name: 'Maria' }),
    ).rejects.toThrow(PetContactNotFoundError)
  })
  ...
  it('lança PetContactNotFoundError se o contato pertence a outro pet', async () => {
    ...
    await expect(
      useCase.execute('user-1', 'pet-1', 'c-1', { name: 'Maria' }),
    ).rejects.toThrow(PetContactNotFoundError)
  })
